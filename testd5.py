...
class LinuxPlusStudyGame:
    ...
    def select_question(self, category_filter=None):
        ...
        if not available_indices:
             # Use COLOR_ERROR for definite errors
    ...
    def display_question(self, question_data, question_num=None, total_questions=None):
        ...
        header_info = f"Category: {category}"
        if question_num is not None and total_questions is not None:
             header_info += f"  |  Question: {question_num}/{total_questions}"
    ...
    def show_stats(self):
        ...
        if not question_stats:
            ...
            def sort_key(item):
                ...
                accuracy = (correct / attempts) if attempts > 0 else 0.5 # Treat 0 attempts as 0.5 for sorting
                return (accuracy, -attempts) # Sort by accuracy ascending, then attempts descending

            sorted_questions = sorted(question_stats.items(), key=sort_key)
    ...
    def run_quiz(self, category_filter=None, mode=QUIZ_MODE_STANDARD):
        ...
        while True:
            ...
            if question_data is None:
                 print(f"{COLOR_INFO} Could not select a question (possibly none left in this filter?). Ending session. {COLOR_RESET}")
                 time.sleep(3)
                 break # Exit the while loop

            question_count += 1 # Increment only if a question was successfully selected
            self.display_question(question_data, question_num=question_count, total_questions="?") # Total unknown until end
            ...
            if user_answer == 'q':
                ...
                # In verify mode, skipping still needs to be recorded maybe? Or just ignored.
                # For now, just continue without processing.
                # In standard mode, we might still update total_questions_session
                if mode == QUIZ_MODE_STANDARD:
                     self.total_questions_session += 1 # Count skipped as attempted in standard
            ...
            if mode == QUIZ_MODE_STANDARD:
                ...
                # Still update history immediately for tracking purposes
                self.update_history(original_question_text, category, is_correct)
                self.total_questions_session += 1
    ...
    def review_incorrect_answers(self):
        ...
        while True:
            ...
            print(f"  {COLOR_OPTION_NUM}c.{COLOR_RESET} {COLOR_WARNING}Clear this incorrect item from review list{COLOR_RESET} (Enter 'c' then the number)")
        ...
        # Final prompt only if the loop wasn't exited by 'b' or EOF
        if current_choice != 'b':
             input(f"\n{COLOR_PROMPT}Press Enter to return to the main menu...{COLOR_RESET}")
    ...
    def display_welcome_message(self):
        ...
        welcome_lines = [
            ...
            "you might encounter on the CompTIA Linux+ certification exam.",
            "",
            "You'll be presented with multiple-choice questions, and",
            "after answering, you'll see an explanation of the correct answer",
            "(in standard quiz mode).",
            "",
            "Let's get started!",
            ""
        ]
    ...
    def main_menu(self):
        ...
        while True:
            ...
            print(f"  {COLOR_OPTION_NUM}1.{COLOR_RESET} {COLOR_OPTIONS}Start a new quiz (all categories){COLOR_RESET}")
            print(f"  {COLOR_OPTION_NUM}2.{COLOR_RESET} {COLOR_OPTIONS}Quiz by category{COLOR_RESET}")
            print(f"  {COLOR_OPTION_NUM}3.{COLOR_RESET} {COLOR_OPTIONS}Verify your knowledge{COLOR_RESET}")
            print(f"  {COLOR_OPTION_NUM}4.{COLOR_RESET} {COLOR_OPTIONS}Review incorrect answers{COLOR_RESET}")
            print(f"  {COLOR_OPTION_NUM}5.{COLOR_RESET} {COLOR_OPTIONS}View your statistics{COLOR_RESET}")
            ...
            # Add Clear Stats as an extra option maybe? Or keep it hidden? Let's add it as 8.
            print(f"  {COLOR_OPTION_NUM}8.{COLOR_RESET} {COLOR_WARNING}Clear Statistics{COLOR_RESET}") # Keep warning color
            cli_print_separator(color=COLOR_BORDER)
...
class LinuxPlusStudyGUI:
    ...
    def _start_quiz_session(self):
        ...
        self._update_status(f"{mode_display} started (Category: {cat_display}).")
    ...
    def _display_question_gui(self):
        ...
        if not self.current_question_data:
            ...
            if self.current_quiz_mode == QUIZ_MODE_STANDARD:
                 final_score_msg = f"\n\nFinal Score: {self.game_logic.score} / {self.game_logic.total_questions_session}"
            ...
            self.question_text.config(state=tk.DISABLED)
            self.quiz_active = False
            self._update_status("Session finished.")
            self.game_logic.save_history() # Save history at the end
            # No automatic messagebox, user sees the message in the main area
            return
    ...
    def _next_question_gui(self):
        ...
        if not self.quiz_active:
             # This shouldn't happen if buttons are managed correctly, but as a safeguard:
        ...
        if question_data is None:
            ...
            # Update status with question number if needed
            q_num = len(self.game_logic.answered_indices_session) # Number of questions *selected* so far
            self._update_status(f"Question {q_num}...")
    ...
    def _submit_answer_gui(self):
        ...
        if self.current_quiz_mode == QUIZ_MODE_STANDARD:
            ...
            self._update_status(f"Answer {self.game_logic.total_questions_session} recorded.")
    ...
    def _show_stats_gui(self):
        ...
        if not sorted_categories:
            stats_text_widget.insert(tk.END, "  No category data yet.\n", "dim")
        else:
            max_len = max((len(cat) for cat in categories_data.keys()), default=10) if categories_data else 10
        ...
        question_stats = history.get("questions", {})
        if not question_stats:
            stats_text_widget.insert(tk.END, "  No specific question data yet.\n", "dim")
        else:
            # Sort questions by accuracy (lowest first) then attempts (highest first)
            def sort_key(item):
                q_text, stats = item
    ...
    def _show_verify_results_gui(self):
        ...
        button_frame.pack(pady=(10, 15))
        close_button = ttk.Button(button_frame, text="Close", command=results_win.destroy, style="TButton", width=12)
        close_button.pack()
    ...
    def _review_incorrect_gui(self):
        ...
        if not_found_questions:
             review_text.insert(tk.END, "\nWarning: Some questions previously marked incorrect could not be found (they might have been removed from the source data):\n", "warning")
                     ...
                     if 0 <= num_to_clear < len(questions_to_review):
                         ...
                         if messagebox.askyesno("Confirm Clear", f"Remove question {num_to_clear+1} from the review list?", parent=review_win):
                              if question_to_clear_text in self.game_logic.study_history["incorrect_review"]:
                                  ...
                                  review_win.destroy() # Close window, user can reopen to see updated list
        ...
        ttk.Button(button_frame, text="Clear Item from List", command=clear_item_from_review_list, style="TButton", width=20).pack(side=tk.LEFT, padx=10)
        ttk.Button(button_frame, text="Close", command=review_win.destroy, style="TButton", width=12).pack(side=tk.LEFT, padx=10)
    ...
    def _quit_app(self):
        ...
        if self.quiz_active:
             ...
             if messagebox.askyesno("Quit Confirmation", "A quiz session is currently active. Are you sure you want to quit?\n(Progress for this session might be lost if not completed)", parent=self.root, icon='warning'):
                 self.game_logic.save_history()
                 self.root.quit()
             ...
             # No active quiz, save and quit directly
             self.game_logic.save_history()
             self.root.quit()
             self.root.destroy() # Ensure window closes fully
